 ...
 */
class RZoomWebinarManagertLite_Sync {

	public function __construct() {
		//Sync Live Zoom Meetings
		add_action( 'wp_ajax_rzwm_sync_user', array( $this, 'sync' ) );
	...
	public function remove_notices() {
		if ( isset( $_GET['page'] ) && $_GET['page'] === "zoom-webinar-sync" ) {
	...
	public static function render() {
		...
		if ( empty( $zoom_api_key ) || empty( $zoom_api_secret ) ) {
			echo '<p>' . __( 'API keys are not configured properly ! Please configure them before syncing.', 'webinar-manager-for-zoom-meetings' ) . '</p>';

			return;
		}

		wp_enqueue_script( 'webinar-manager-for-zoom-meetings-js' );
		wp_localize_script( 'webinar-manager-for-zoom-meetings-js', 'rzwm_sync_i10n', array(
			'before_sync'              => __( "Fetching data.. Please wait this might take some time depending on how many Zoom Meetings you have.", "webinar-manager-for-zoom-meetings" ),
			'total_records_found'      => __( "Total Records Found", "webinar-manager-for-zoom-meetings" ),
			'total_not_synced_records' => __( "Total Not Synced Records", "webinar-manager-for-zoom-meetings" ),
			'select2_placeholder'      => __( "Choose meeting from here to sync.", "webinar-manager-for-zoom-meetings" ),
			'sync_btn'                 => __( "Sync Now", "webinar-manager-for-zoom-meetings" ),
			'sync_start'               => __( 'Starting sync process.. Please wait for some time. Do not close this window until completed.', "webinar-manager-for-zoom-meetings" ),
			'sync_error'               => __( 'Opps ! You have not selected any meeting to sync yet. Select one or two and this section will be filled by happiness for you !!', "webinar-manager-for-zoom-meetings" ),
			'sync_completed'           => __( 'Meeting sync has been completed !', "webinar-manager-for-zoom-meetings" ),
		) );

		require_once RZWM_PLUGIN_DIR_PATH . '/includes/views/sync.php';
	}
	...
	public function sync() {
		...
		$zoom_api_secret = get_option( 'zoom_api_secret' );
		if ( empty( $zoom_api_key ) || empty( $zoom_api_secret ) ) {
			wp_send_json_error( __( 'API keys are not configured properly ! Please configure them before syncing.', 'webinar-manager-for-zoom-meetings' ) );
		}
		...
		if ( $type === "check" ) {
			...
			if ( ! empty( $meetings ) ) {
				...
				if ( ! empty( $meetings['meetings'] ) ) {
					...
					update_option( '_rzwm_sync_meetings', json_encode( $meetings ) );
					wp_send_json_success( $meetings );
				} else {
					wp_send_json_error( __( "No Meetings Found !", 'webinar-manager-for-zoom-meetings' ) );
				}
			} else {
				wp_send_json_error( __( "No Meetings Found !", 'webinar-manager-for-zoom-meetings' ) );
			}
		...
		if ( $type === "sync" ) {
			...
			if ( ! empty( $meeting_id ) && ! in_array( $meeting_id, $db_meetings ) ) {
				$cached_meetings = json_decode( get_option( '_rzwm_sync_meetings' ) );
				...
				$data = array(
					'msg'        => __( "No meeting is selected or selected meeting already exists", 'webinar-manager-for-zoom-meetings' ) . ': <strong>' . $meeting_id . '</strong>',
	...
	private function create_meeting( $meeting_obj ) {
		...
		if ( ! empty( $post_id ) ) {
			...
			$mtg_param = array(
				...
				'start_date'                => rzwm_dateConverter( $meeting_obj->start_time, $meeting_obj->timezone, 'Y-m-d H:i', false ),
			...
			#do_action( 'rzwm_admin_after_zoom_meeting_is_created', $post_id, false );

			$data = array(
				'msg'        => __( "Successfully imported meeting with ID", 'webinar-manager-for-zoom-meetings' ) . ': <strong>' . $meeting_obj->id . '</strong>',
				'meeting_id' => $meeting_obj->id
			);
			wp_send_json_success( $data );
		} else {
			$data = array(
				'msg'        => __( "Failed to import meeting with ID", 'webinar-manager-for-zoom-meetings' ) . ': <strong>' . $meeting_obj->id . '</strong>',
				'meeting_id' => $meeting_obj->id
			);
			wp_send_json_success( $data );
		}
...
new RZoomWebinarManagertLite_Sync();